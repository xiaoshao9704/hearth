...
| `ingest_provider` 推流入口 | OBS WHIP 接入 | 内建 `bellows`（Bellows，进程内直通网关，支持 HEVC/AV1，默认）；或任一已注册 `livekit` 实例（走它自带的 WHIP 入口，推流不必多绕一跳）；或已注册 `livekit-ingress` / `bellows-remote` 实例 |
