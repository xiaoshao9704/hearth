...
- OBS WHIP 推流：每用户一把推流令牌、频道写在 URL 里（`/providers/{alias}/w/<频道 id>`，换房间只改服务器地址，令牌不动）；地址与令牌在频道菜单（房间顶栏的频道名）的「OBS 推流地址…」一键复制，旧的 `…/w/<频道名>` 地址继续有效。服务端不转码原样透传（2K/4K/120fps）
...
- 访客：频道管理员发访客链接，无账号也能进指定频道（绑这台浏览器、有寿命、过期清理）；**访客转正为注册账号默认关闭**，管理后台把 `guest_claim` 设为 `on` 才开放（转正后 user_id 不变，聊天记录与频道里的位置都留下）
- 通行密钥（Passkey）：一次触摸/面容/指纹即登录，不用输用户名（可发现凭证）；设置「账户」里可加多枚、改名、删除，密码始终保留作兜底
- 通知：被 @ 与被回复同一档提醒（提示音 + 系统通知 + 离线推送），未读数上标签页标题与图标角标；每人每频道可单独静音（见下）
...
| 层 | 何时响 | 前提 |
|---|---|---|
| 提示音 | 页面开着（含被 @、被回复、进出房间） | 无 |
| 系统通知 | 页面在后台或最小化 | 浏览器给了通知权限；经 Service Worker 弹出，点通知直达该频道 |
| 离线推送 | **页面关着**，且是被 @ 或被回复 | 授权了系统通知（默认自动订阅）；可在设置「语音与视频 → 系统通知」里关掉 |

**只推被 @ 与被回复**，普通消息不推——离线推送的价值在"有人点名找我"。

**通道由浏览器决定**：Chrome 系走 Google、Firefox 走 Mozilla、Edge 走微软、Safari/iOS 走 Apple。hearth 只按 VAPID 把密文交给浏览器给出的推送地址，不做任何厂商特判，送不到就静默。这意味着：

- **服务端必须能出网访问对应厂商的推送网关**，否则推送发不出去（页面内通知不受影响）。
- **iPhone / iPad 必须先「添加到主屏幕」**，从那个图标打开后才有推送权限；普通 Safari 标签页里连开关都点不开（设置里会写明原因）。
- 图标角标（未读数画在应用图标上）只在装成 PWA 或支持该 API 的桌面浏览器上看得见。
- 站点必须是 https（`localhost` 例外）——Service Worker 与推送都是浏览器的硬约束。

**密钥不用配**：`webpush_vapid_public` / `webpush_vapid_private` 首次用到时自动生成一对并落库（随数据库备份），管理后台「管理」分组里只读回显公钥。改这两个键会让**所有已有订阅立即失效**（各设备下次打开页面时自动重新订阅）。`webpush_subject` 是写进 VAPID 断言里给网关的联系方式，留空即 `mailto:admin@<当前请求的 Host>`。

**频道静音**：每人每频道一个开关，入口在大厅频道卡片的铃铛按钮与房间里「设置 → 语音与视频 → 系统通知」。静音 = 这个频道**不发任何**提示音/通知/推送（被 @ 与被回复也不例外），未读数照算、侧栏与卡片上带一个划掉的铃铛。

**退出登录即退订**：订阅绑在会话上，退出登录或在「账户」里远程下线某台设备时，那台设备的订阅一起删掉。
...
「晚上卡」到底卡在哪一段，房间里有三处读数，都在浏览器内完成，不需要额外部署：

- **连接质量面板**：顶栏的连接 chip 点开，每条线（语音 / 舞台）一行 RTT · 抖动 · 丢包 · 传输方式（UDP / TCP 兜底 / 中继兜底），5 秒刷新。投屏卡片的统计条同时给出收侧「缓冲 xx ms · 解码 xx ms」与发侧「编码 xx ms · 受限：带宽」。进房后每 60 秒还会往服务端日志写一条 `event="line_stats"` 的诊断（页面切到后台就停）。
- **参与者连接质量**：内核按 RTCP 回报算出的档位，差到 poor/lost 时名册里那台设备名字旁出现信号弱图标；OBS 推流在浏览器之外，它的上行好坏就看推流角标里那行「上行：差」。
- **端到端延迟标尺**：新开一个标签页打开 `#/latency`，页面把服务器对齐过的时间画成一行黑白色码。推流方用 OBS 抓这个窗口、或在浏览器投屏里共享这个标签页；任何观众在那路画面上点卡片右上角「更多操作 → 测延迟」，页面从视频帧里读回时间戳与自己校准过的时钟相减，10 秒后给出「端到端 480 ms（p95 610，31 样本，时钟 ±12 ms）」——采集、编码、服务器转发、解码、渲染全程都在里面。两端各自把时钟对齐到服务器（`GET /api/time`），不必同机、也不必对表。
