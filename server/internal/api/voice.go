...
// 这里验票后把连接交给 pionvoice.Provider 处理（阻塞至连接结束）。
package api
...
func (a *API) voiceWS(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("pion-voice 入会: room=%s identity=%s", tk.room, tk.identity)
	// hijack 后 r.Context() 会被 net/http 取消，连接生命周期用独立 context
	a.pion.HandleJoin(context.WithoutCancel(r.Context()), tk.room, tk.identity, tk.name, tk.muted, conn)
}
