...
export async function renderRoom(root: HTMLElement, channel: string) {
  ...
  // 壳内「选中即开播」：目标已经写进 OBS 的源，这里只把本频道的 WHIP 地址与令牌配上去并开播。
  // 地址与令牌按需现拉（面板可能在频道刚就绪时就打开了），不在房间页常备一份。
  async function startObsToChannel() {
    const c = obsConn();
