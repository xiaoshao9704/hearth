...
export async function renderJoin(root: HTMLElement, code: string) {
  ...
  function paintForm() {
      ...
      <form style="margin-top:20px;display:flex;flex-direction:column;gap:13px" id="join-form">
        <div style="display:flex;flex-direction:column;gap:7px">
          <div style="display:flex;align-items:baseline;gap:8px">
            <div class="field-label" style="flex-grow:1">用户名</div>
            <div id="hint-user" style="font-size:11px"></div>
          </div>
          <div class="field" style="height:46px"><input id="jn-user" placeholder="2–32 位，字母数字 - _" autocomplete="username" /></div>
        </div>
        <div style="display:flex;flex-direction:column;gap:7px">
          <div style="display:flex;align-items:baseline;gap:8px">
            <div class="field-label" style="flex-grow:1">密码</div>
            <div id="hint-pass" style="font-size:11px"></div>
          </div>
          <div class="field" style="height:46px">
            <input id="jn-pass" type="password" placeholder="至少 8 位" autocomplete="new-password" />
        ...
        <div style="display:flex;flex-direction:column;gap:7px">
          <div style="display:flex;align-items:baseline;gap:8px">
            <div class="field-label" style="flex-grow:1">确认密码</div>
            <div id="hint-conf" style="font-size:11px"></div>
          </div>
          <div class="field" style="height:46px"><input id="jn-conf" type="password" placeholder="再输一次" autocomplete="new-password" /></div>
        </div>
        <p class="error-text" id="jn-error" style="margin:0;min-height:0"></p>
        <button type="submit" class="hit btn btn-primary btn-lg disabled" id="jn-btn" style="margin-top:4px">创建账号并进入</button>
        <div class="notice-ok hidden" id="jn-done">${icon('check', 16, 'var(--sage)', 1.9)}<span>账号建好了，正在带你进大厅…</span></div>
      </form>`;
    ...
    function sync() {
      ...
      btn.classList.toggle('disabled', !valid() || busy || done);
    }
  ...
  const timer = setInterval(() => {
    if (!root.isConnected || !location.hash.startsWith('#/join/')) {
      clearInterval(timer);
      return;
    }
    paintTTL();
    if (!alive() && body.querySelector('#join-form') && !done && !busy) paintExpired();
  }, 1000);
}
