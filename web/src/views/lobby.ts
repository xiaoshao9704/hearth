...
export function isLobbyHash(hash: string): boolean {
  return (
    hash === "" || hash === "#/" || hash === "#/lobby" || hash === "#/channels"
  );
}

function statusOf(online: number, inviteOnly: boolean): string {
  if (online <= 0)
    return inviteOnly ? "邀请制 · 现在没人" : "空着，进去就是你的";
...
function cardHtml(c: Channel): string {
  ...
  const roleTag =
    c.my_role === "owner"
      ? '<span class="tag">我的频道</span>'
      : c.my_role === "moderator"
        ? '<span class="tag">我管理的</span>'
        : "";
  const hiddenTag = c.hidden ? '<span class="tag tag-red">隐藏中</span>' : "";
  const mutedTag = c.muted
    ? `<span class="tag" title="已静音">${slashIcon("bell", 11, true, "currentColor")}静音</span>`
    : "";
...
async function maybeNudgePasskey(
  host: HTMLElement,
  alive: () => boolean,
): Promise<boolean> {
...
const INSTALL_COPY: Record<
  "prompt" | "ios" | "mac-safari" | "manual",
  { title: string; body: string; primary?: string }
> = {
  prompt: {
    title: "安装到桌面",
    body: "像应用一样独立打开，带图标和角标，通知更可靠。",
    primary: "安装",
  },
  ios: {
    title: "添加到主屏幕",
    body: "Safari 里点底部「分享」→「添加到主屏幕」。装好后从主屏幕打开，才能收到通知。",
  },
  "mac-safari": {
    title: "安装到程序坞",
    body: "Safari 菜单「文件」→「添加到程序坞」。",
  },
  manual: {
    title: "安装到桌面",
    body: "在浏览器菜单里找「安装」或「添加到主屏幕」。",
  },
};

// maybeNudgeInstall 进大厅时的安装推荐卡，挂在通行密钥卡下面。条件：非独立窗口运行、
// 当前平台有安装路径（installMode() !== 'none'）、本设备没关掉过、本会话没弹过。
async function maybeNudgeInstall(
  host: HTMLElement,
  alive: () => boolean,
): Promise<boolean> {
  let shown = false;
  ...
  const paint = (m: InstallMode) => {
    ...
    host
      .querySelector("#install-nudge-do")
    ...
    host
      .querySelector("#install-nudge-later")!
      .addEventListener("click", () => {
        setNudge(INSTALL_NUDGE_KEY, String(Date.now()));
        host.innerHTML = "";
      });
    host
      .querySelector("#install-nudge-never")!
      .addEventListener("click", () => {
...
export async function renderLobby(root: HTMLElement, alive: () => boolean) {
  ...
  void (async () => {
    const shownPasskey = await maybeNudgePasskey(
      root.querySelector<HTMLElement>("#passkey-nudge")!,
      alive,
    );
    if (!shownPasskey)
      await maybeNudgeInstall(
        root.querySelector<HTMLElement>("#install-nudge")!,
        alive,
      );
  })();
  ...
  root
    .querySelector("#tune-av")!
    .addEventListener("click", () => openSettings("av"));
  root
    .querySelector("#guest-bar")
    ?.addEventListener("click", () => openSettings("account"));
  ...
  async function paint() {
    ...
    try {
      ...
      cardsEl
        .querySelector("#cards-retry")!
        .addEventListener("click", () => void paint());
      return;
    }
    if (!alive()) return;
    ...
    if (channels.length === 0) {
      ...
      cardsEl
        .querySelector("#cards-empty-create")
        ?.addEventListener("click", () => {
