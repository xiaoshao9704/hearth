...
状态：2026-09-15 晚方向调整。原生投屏在 Windows 真机上不如 OBS 流畅，且 OBS 联动能覆盖投屏与程序音频，桌面端收缩为薄壳。见 [桌面端方案](plan-desktop.md)。

- 投屏体验改由网页端「OBS 联动」承担（obs-websocket 一键配置本机 OBS 推 WHIP），浏览器与桌面端共用。
- 桌面端保留：应用内信任、浏览器登录、单实例与深链、本机服务、全局热键与托盘；原生采集与 GStreamer 暂停并移出默认构建。
- 已落地的验证结论（WHIP 入口零改动、应用内信任、设备票、浏览器登录）继续有效。节点 5 的「全局热键小托盘程序」即薄壳。
