...
状态：**三个阶段均已落地**（阶段一 2026-09-04，阶段二与三 2026-09-07）。

落地时与本文两处不同：升级接口叫 `POST /api/account/claim`（不是 `upgrade`），且**两种访客都能转正**
——「频道访客不能升级」这条限制没有保留：转正后 user_id 不变、频道白名单里的那行也还在，
不给这条路只会逼对方另注册一个账号，反倒把聊天记录和位置丢了。

转正能力本身**默认关闭**：站点要在管理后台把 `guest_claim` 置为 `on` 才开放（默认 `off` 时
`POST /api/account/claim` 返回 403，前端三处入口只报剩余时间）。缺省不开是因为「访客链接」
的常见用法是一次性带人进房，顺手把访客变成常驻账号未必是站点想要的。
