...
状态：P1 与级联（第六节）已实现。取代 `docs/plan-bellows-upnp.md`（那份只覆盖远端 Bellows 一个进程，
...
内层网关给出的外部地址是私网时自动向上游那一层继续申请（`portmap/chain.go`），
级联不成才退回上面这段的人工配置。实现如下：

- **形态是链式 client**：`chainClient` 的 `hops[0]` 是默认网关，之后每跳是上一跳的上游；
  逐跳把「上一跳给出的外部端口」作为下一跳的内部端口申请（同端口优先），
  `Mapper` 只看最外层的外部地址与端口，不感知跳数。撤销时从最外层往里删——
  外层映射的内部端口正是内层的外部端口，内层先没了会让外层指向一个已不存在的端口。
- **上游看到的请求者就是内层路由**：PCP 请求头的 client IP（`pcpClient.clientIP`）、
  UPnP 的 `NewInternalClient`（`upnpClient.internalClient`）都填内层路由的 WAN 地址——
  那本来就是报文到达上游时的源地址，既过得了 PCP 的 `ADDRESS_MISMATCH` 校验，
  也过得了 miniupnpd `secure_mode`「只能给自己开洞」这条（第五节）。不需要 THIRD_PARTY option。
- **发现上游**：候选地址按内层 WAN 地址所在 /24 的 `.1` 启发（它自己就是 `.1` 时改试 `.254`）；
  先 PCP/NAT-PMP 单播，再对该地址做**单播 SSDP**（`discoverUPnPAt`，M-SEARCH 直接发给它，
  绕开多播出不了本级路由的问题）。与 `Mapper.pick` 同一思路：**用一条真实申请当探测**，
  成功即锁定这一跳，只有 `ErrUnsupported` 才换下一个协议——「声称支持」不算数（见下）。
  无特权 traceroute 兜底可以后置，不是先决条件。
- **护栏**：候选地址必须是私网/CGNAT，绝不向公网地址发映射请求；上游地址与链上已有某跳的
  网关地址重合（环路）即停；**深度上限 3 跳**（含默认网关），任一跳拿到的外部地址已是公网即停。
- **发现失败带负缓存**：同一个内层外部地址在 10 分钟（`maxBackoff`）内不重复发现，
  日志也只打一次。上游不开 UPnP/PCP 是常态，续租轮不该每次都付一遍发现的耗时。
- 某一跳失败就退回单跳诊断（即 P1 的行为），纯增量：`upstream_nat` 的文案在原有指引前
  多一句「已尝试向上游网关申请但未成功（原因）」，指引里要转发的地址取**最外层已到达那跳**
  的外部地址。级联成功时 `Status.Hops` 回显每一跳（网关、方法、该跳的外部地址）。
...
- **内层 WAN 地址变了怎么办**：上游那跳的凭据（client IP / NewInternalClient）就是这个地址，
  变了之后原样重发会被上游判成地址不符。链记下每跳是为哪个内层地址建的，不一致就丢掉那跳往外的部分重新发现。
...
- **P1 之后**：级联申请（第六节，已实现）。
- **P2**：远端 Bellows 状态接口（见第三节）+ healthz 与管理后台「网络」区（含 `Status.Hops` 的跳数回显）；PCP 的 v6 pinhole；`PUBLIC_URL` 一键填入。
