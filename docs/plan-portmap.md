...
状态：方案待评审。2026-09-03。取代 `docs/plan-bellows-upnp.md`（那份只覆盖远端 Bellows 一个进程，
且其中「改 SetNAT1To1IPs 语义」一条已由 `lite.Announcer` 的地址改写规则实现）。
...
- `cmd/bellows`：同一个包，映射 `BELLOWS_UDP_PORT`（StrictPort）与 HTTP 端口，取代 `plan-bellows-upnp.md`。
- `cmd/bellows`（P1 一并做）：映射 `BELLOWS_UDP_PORT` 与 HTTP 端口，宣告同样走上面的候选追加。
  远端形态跑在别人的局域网里，**是最需要自动映射的场景**。
- **远端 Bellows 状态接口**（P2）：远端进程没有数据库也不回调 hearth，它的映射成败 hearth 无从得知，
  而管理后台恰恰要展示。做法是扩展远端已有的 `/healthz`：基础探活保持匿名且恒 200（不得因映射失败返回非 200，
  否则 autoheal 会误杀）；详细状态（方法、每条映射、诊断分类、宣告地址）需**用共享密钥签名**才返回——
  它暴露的是对方的内网拓扑，不能裸奔。hearth 在管理后台渲染 bellows-remote 实例时拉一次，
  连不上本身也是有用的信息（「hearth 访问不到远端的 HTTP 端口」）。
...
「向两层各申请一次」理论上免去上游配置，但不做，理由：

- **发现不了上游网关**：SSDP 是多播，出不了本级路由；PCP 只能发给默认网关。要定位上游只能靠 TTL 探测
  （TTL=2 看谁回 ICMP time-exceeded），而收 ICMP 需要额外权限，容器内不保证可用。
- 两层各自续租，任一层失效整条链即断，还要处理部分成功的回滚，复杂度翻倍而失败点加倍。
- 运营商侧设备的 UPnP 实现质量参差，且常带远程管理，配置可能被上游重置。
- 端口链对齐反而不是障碍（先申请内层拿到中间端口，再用它去申请外层），但不足以抵消上面三条。

替代做法是**诊断 + 一句可执行的指引**：识别出上游还有一层时，明确告诉用户
「请在上游设备把 <端口> 转发到 <本机网关的 WAN 地址>，或开启 DMZ」。一次配好的静态转发，
比一条可能在任意一层悄悄失效、用户还不知道该查哪儿的自动链条更可靠。
...
- `/healthz` 在现有 `announce` 段旁加 `portmap` 段，沿用同一回显结构。
