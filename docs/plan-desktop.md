...
状态：**方向已定、关键假设已验证（2026-09-15），里程碑 1（macOS 纵切）实施中。**
...
| Tauri 接线 | 未验证 | wry 的 navigation delegate 不含挑战回调，也没有公开 API。需 `with_webview` 拿到 WKWebView 后挂一个只实现挑战方法、其余转发给原 delegate 的代理对象；直接顶掉会丢 IPC 初始化脚本与下载路由。 |
