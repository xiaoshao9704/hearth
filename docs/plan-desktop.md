...
状态：**2026-09-15 晚方向调整：桌面端收缩为薄壳，原生采集暂停；投屏体验改由网页端「OBS 联动」承担。** M1/M2 的原生采集代码留在分支上不删，不再作为交付项。
对应 [roadmap](roadmap.md) 的提前技术验证节点，不指定发布版本号。
下文「已验证」指本仓库 2026-09-15 的实测或官方文档核实，「未验证」项不得当作承诺。

## 定案

### 2026-09-15 晚：方向调整（用户拍板）

Windows 真机对比：桌面端原生投屏不如 OBS 流畅（OBS 同样推 H.265，观众端相同，差在推流端管线）；用户复盘后认定，只要网页能一键配置本机 OBS 推流，桌面端在投屏上就没有不可替代的价值。逐项对照：

| 能力 | 浏览器 + OBS 联动 | 桌面端独有？ |
| --- | --- | --- |
| 流畅投屏、固定码率硬编、独占全屏游戏捕获 | OBS 本身 | 否 |
| 指定程序音频 | OBS 28 起自带应用音频采集 | 否 |
| 自签证书免装根 | 网页与 OBS 的 WHIP 都走系统信任库，都要装根 | 只省网页那一半 |
| 全局按键说话（游戏全屏时） | 浏览器做不到 | **是** |
| 托盘常驻、开机自启 | 做不到 | **是** |
| 一键在本机跑服务器 | CLI 或安装包也能 | 便利性 |

决定：

- **OBS 联动优先**，纯网页实现（obs-websocket 5.x，`SetStreamServiceSettings` 的 `whip_custom` + `StartStream`），浏览器与桌面端共用。只能配置与浏览器同机的 OBS；OBS ≥ 30。
- **桌面端收缩为薄壳**：WebView、应用内信任、浏览器登录、单实例与深链、全局热键、托盘、本机服务。去掉 GStreamer 与原生采集的打包（原生采集代码收进 cargo feature，默认不编译、不带运行时；包体回到几 MB）。
- **原生采集暂停**：不做 GPU 管线改造；「不想装 OBS」的需求出现再评估。
...
| GStreamer 分发 | 已验证 | Homebrew `gstreamer` 的 nice 插件是悬空链接，需 `libnice-gstreamer`；分发包必须自带。1.24 起官方 Windows/macOS 二进制含 gst-plugins-rs，Windows ARM64 除外。无官方最小裁剪配方，完整包约几十 MB。**macOS 已自包含**：`scripts/bundle-gst-macos.sh` 按实测加载清单挑插件、顺依赖搬库进 `.app`，`env -i` 隔离启动实测推流/预览通过，无 `/opt/homebrew` 运行时依赖。**Windows 已裁剪**：`scripts/bundle-gst-windows.ps1` 同样按显式插件清单 + `dumpbin` 依赖闭包收敛官方 runtime，CI 逐元素 `gst-inspect` 与管线自检。 |
...
真机验收（2026-09-15，ad-hoc 签名包，授予屏幕录制与本地网络权限后）：SCK 真实采集经 WHIP 到达浏览器观众，**投屏画面通过，无明显延迟**。ad-hoc 签名按每次构建记账，重新打包后两项授权要重授。所属应用音频是否到达观众未单独确认。
