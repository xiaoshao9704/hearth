...
- 完整官方 runtime，包括 WGC、WASAPI2、Media Foundation、NVENC、QSV、AMF、转换/缩放、预览 JPEG、appsrc/appsink、Opus、解析器、WHIP 插件及其依赖。硬件编码器是否注册取决于实际 GPU 和驱动，不能由 CI runner 代替验证。
- 官方 runtime 的 `share`、`lib`、`libexec`、`etc` 等目录按原样保留，只排除构建机 SDK 的 Inno 卸载器。Cerbero 将 `share/licenses` 归入 devel，脚本会在开发安装完成后额外复制这个完整目录，携带各组件的 license/copyright 材料；后续裁剪不得遗漏。源码与版权信息以随包的官方材料为准。
...
2. 固定下载哈希匹配；WGC/WASAPI2/各硬编插件、转换缩放与预览依赖、官方许可证目录齐全。
3. 安装后的完整运行时逐文件 SHA-256 与打包清单一致，主 exe 旁 DLL 与原版一致。
4. 清掉构建机 GStreamer 环境与插件缓存后，使用包内 `gst-inspect` 检查所需元素、执行转换/缩放测试管线。
